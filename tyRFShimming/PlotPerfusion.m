...
if isempty(matched_ptx) || isempty(matched_CP)
    ...
    CutOff = prctile(AllPoints,99);
    figure(99)
    set(gcf,'color','w','InvertHardcopy','off')
    set(gcf,'units','centimeters','position',[4 4 40 20],'paperunits','centimeters','paperposition',[0 0 40 20])
    clf
    subplot(1,2,1)
    imagesc(CP_Img.image',[0 CutOff]);colormap('gray');axis off;title('CP mode','FontSize',14)
    subplot(1,2,2)
    imagesc(pTx_Img.image',[0 CutOff]);colormap('gray');axis off;title('RF shimming','FontSize',14)
