...
for iDx = 1:numel(string_separated)-1
    ...
    %   First find out the max peak power and rthe 6min and 10s avg power
        ...
        info_extracted(IdxCount).RF.pusle_amp = zeros(1,num_of_pulses);
        ...
        for iDx2 = 1:num_of_pulses
            ...
            if ~isempty(The_Line)
                ...
                TempStr = TempStr{1}{1};
                TempStr(1:2) = [];
                info_extracted(IdxCount).RF.pusle_amp(iDx2) = str2double(TempStr);
            end
        end
...
%                 TempStr = TempStr{1};
%                 TempStr(1:2) = [];
%                 info_extracted(IdxCount).RF.pusle_amp(RF_index) = str2double(TempStr);
%             end
% 
%         end
    end
